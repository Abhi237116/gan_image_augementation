...
def self_learn(
    ...
    for i in range(start, num_iters):
        ...
        for j in range(num_gans):
            new_data = (
                gans[j]
                .generator(torch.randn(num_new_images, gans[j].gen_input_dim))
                .detach()
            )
            next_dataloaders[j] = torch.utils.data.DataLoader(
                torch.cat((cur_dataloaders[j].dataset, new_data), 0),
