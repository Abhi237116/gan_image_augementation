...
class DeepClassifier(torch.nn.Module):
    def __init__(self, lr=0.0002, input_dim=784, output_dim=10, dropout_rate=0.1):
        """
        3 hidden layers
        ...
        self.hidden1 = nn.Sequential(
            nn.Linear(input_dim, 512), nn.LeakyReLU(0.01), nn.Dropout(dropout_rate)
        )
    ...
    def train(self, data_loader, num_epoch=100, synth=False):
        ...
        utils.plot_loss_2(
            self.lst_epochs,
            self.lst_loss,
            "classifier_loss_synth" if synth else "classifier_loss_orig",
        )
...
class SimpleClassifier(torch.nn.Module):
    def __init__(
        ...
        self.hidden = nn.Sequential(
            nn.Linear(input_dim, hidden_dim), nn.Sigmoid(), nn.Dropout(dropout_rate)
        )
    ...
    def train(self, data_loader, name, num_epoch=100):
        ...
        utils.plot_devset_accuracy(
            self.lst_epochs, self.lst_dev_accuracy, name + "_acc"
        )
