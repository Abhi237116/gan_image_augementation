...
class Discriminator(torch.nn.Module):
    def __init__(self, input_dim, output_dim=1, dropout_rate=0.1, alpha=4):
        """
        ...
        self.output = nn.Sequential(nn.Linear(int(8 * alpha), output_dim), nn.Sigmoid())
...
class Generator(torch.nn.Module):
    def __init__(self, input_dim, output_dim=784, dropout_rate=0.1, alpha=4):
        """
        no dropout
        3 hidden layers
        """
        ...
        self.output = nn.Sequential(nn.Linear(int(128 * alpha), output_dim), nn.Tanh())
